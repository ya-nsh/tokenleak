...
export interface WrappedLiveServerOptions {
  port?: number;
}
...
/**
 * Start a local HTTP server that renders the AI Wrapped presentation as an interactive HTML page.
 * Finds a free port starting from the given port (default 3456).
 */
export async function startWrappedLiveServer(
  output: TokenleakOutput,
  options: WrappedLiveServerOptions = {},
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (result.server) {
      ...
      process.stderr.write(
        `AI Wrapped live at http://localhost:${String(actualPort)}\n`,
      );
